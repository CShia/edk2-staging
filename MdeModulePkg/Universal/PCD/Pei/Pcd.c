  ...
  )
{
  ...
  ASSERT_EFI_ERROR (Status);
  Length = (*(UINT32 *)((UINT8 *) FileHandle + sizeof (EFI_FFS_FILE_HEADER))) & 0xFFFFFF;
  ...
  )
{
  ...
  if (Index < SkuIdTable[0]) {
    ...
    ASSERT_EFI_ERROR (Status);
    Length = (*(UINT32 *)((UINT8 *) FileHandle + sizeof (EFI_FFS_FILE_HEADER))) & 0xFFFFFF;
    Index  = ((PEI_PCD_DATABASE *) PcdDb)->Length;
    SkuDelta = NULL;
    while (Index < Length) {
    ...
    if (Index < Length && SkuDelta != NULL) {
      SkuDeltaData = (DATA_DELTA *) (SkuDelta + 1);
      while ((UINT8 *) SkuDeltaData < (UINT8 *) SkuDelta + SkuDelta->Length) {
        *((UINT8 *) PcdDb + SkuDeltaData->Offset) = (UINT8) SkuDeltaData->Value;
        SkuDeltaData ++;
      }
